...
from config import Chiku_MONGO_HOST

my_client = MongoClient(host=Chiku_MONGO_HOST)
my_db = my_client["Chiku-db"]
...
def getLanguage(user_id: int) -> str:
    ...
    """
    my_col = my_db["users"]
    return my_col.find_one({"user_id": user_id})["Chiku_lang"]
...
def updateLanguage(user_id: int, lang_code: str) -> None:
    ...
    """
    my_col = my_db["users"]
    my_col.update_one({"user_id": user_id}, {"$set": {"Chiku_lang": lang_code}})
